...
class SwiftGenerator {
    ...
    **/
    ...
    static function generateSwift(cls:haxe.macro.Type.ClassType, outputDir:String, ?modelTypes:Map<String, haxe.macro.Type.ClassType>, ?componentTypes:Map<String, haxe.macro.Type.ClassType>):Void {
        ...
        if (hasGlance) {
            ...
            appSwift.add("            if phase != .active { sui_glance_resample() }\n");
            appSwift.add("        }\n");
        }
    ...
    **/
    ...
    /**
        ...
        Display only, for now. A tap in a WidgetKit widget is an `AppIntent`,
        and an intent runs in a process where the closures are not — which is
        a different problem from Android's, where the tap already lands in the
        application's own process. The action ids are in the tree, waiting.
    **/
    static function generateGlanceWidget(className:String):String {
        var lines = [
            ...
            "        // A Button draws its label. Its action id is in the tree and",
            "        // nothing sends it back yet — see the class doc.",
            "        case \"Button\": Text(props?[\"label\"] as? String ?? \"\").bold()",
